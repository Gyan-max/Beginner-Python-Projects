...
class RockPaperScissorsGame:
    def __init__(self, window):
        ...
        # Load images
        self.rock_image = Image.open("rock.png").resize(
            (100, 100), resample=Image.BICUBIC
        )
        self.rock_photo = ImageTk.PhotoImage(self.rock_image)

        self.paper_image = Image.open("paper.png").resize(
            (100, 100), resample=Image.BICUBIC
        )
        self.paper_photo = ImageTk.PhotoImage(self.paper_image)

        self.scissors_image = Image.open("scissor.png").resize(
            (100, 100), resample=Image.BICUBIC
        )
        ...
        # Buttons
        # self.buttons = []
        # for image, label_text in [(self.rock_photo, "Rock"), (self.paper_photo, "Paper"), (self.scissors_photo,
        #                                                                                    "Scissors")]:
        #     button = tk.Button(self.button_frame, image=image, bg="pink", bd=0, fg="pink", command=lambda img=image:
        #                             self.determine_winner(img))
        #     button.image = image
        ...
        # Buttons
        self.buttons = []
        for image, label_text in [
            (self.rock_photo, "Rock"),
            (self.paper_photo, "Paper"),
            (self.scissors_photo, "Scissors"),
        ]:
            button = tk.Button(
                self.button_frame,
                image=image,
                ...
                bd=0,
                fg="pink",
                command=lambda img=image: self.determine_winner(img),
            )
            button.image = image
            button.pack(side=tk.LEFT, padx=10)
        ...
        self.result_label = tk.Label(
            window, text="", font=("Arial", 14), bg="pink", fg="#8b4513"
        )
        self.result_label.pack(pady=10)

        # Score label
        self.score_label = tk.Label(
            window, text="", font=("Arial", 12), bg="pink", fg="#8b4513"
        )
    ...
    def determine_winner(self, user_choice):
        ...
        if user_choice_name == computer_choice:
            ...
            (user_choice_name == "Rock" and computer_choice == "Scissors")
    ...
    def update_score_label(self):
        score_text = (
            f"Your Score: {self.user_score} | Computer Score: {self.computer_score} | Rounds Played: "
